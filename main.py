...
CSV_FILE = os.path.join(BASE_DIR, "data", "raw", "Bank", "Bank Customer Churn Prediction.csv")
...
# Категориальные признаки, как в обучении
cat_features = ['country', 'gender', 'active_member']

@app.route("/predict")
def predict():
    X = clients.drop(columns=["client_id"])
    # CatBoost корректно обрабатывает категориальные признаки
